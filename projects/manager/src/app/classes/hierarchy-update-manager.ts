...
export class HierarchyUpdateManager extends ListUpdateManager {
    ...
    addOtherItem(otherArray: Array<HierarchyItem>, thisIndex: number, thisHierarchyItem: HierarchyItem) {
        ...
        if (!addedItem) {
            ...
            if (thisHierarchyItem.hierarchyGroupID == 0) {
                ...
                if (this.hasChildren(otherArray[indexOfOtherParent], otherArray)) {
    ...
    onSearchItemDelete(searchUpdate: MultiColumnListUpdate) {
        ...
        if ((searchUpdate.deletedItems![0] as MultiColumnItem).values[1].name == this.parentSearchType) {
            ...
            this.deleteChildren(this.thisSearchArray, (searchUpdate.deletedItems![0] as MultiColumnItem), this.thisArray);
        }
    ...
    deleteOtherItem(list: Array<ListItem>, deletedItem: ListItem, type: number | string) {
        ...
        window.setTimeout(() => {
            ...
            const index: number = typeof type == 'number' ? list.findIndex(x => x.id == deletedItem.id && x.hierarchyGroupID == type) : list.findIndex(x => x.id == deletedItem.id && (x as MultiColumnItem).values[1].name == type);
        ...
        if (this.otherProductArray) {
            this.productService.productComponents.forEach(x => {
                ...
                if (update.type == ListUpdateType.Edit) {
                    ...
                    // From Hierarchy
                    if ((update as HierarchyUpdate).hierarchyGroupID != null) {
                        if ((x[this.otherProductArray] as Array<HierarchyItem>).length > 0) this.editOtherItem(x[this.otherProductArray] as Array<HierarchyItem>, update, (update as HierarchyUpdate).hierarchyGroupID);
                        if ((x[this.otherProductSearchArray] as Array<MultiColumnItem>).length > 0) this.editOtherItem(x[this.otherProductSearchArray] as Array<MultiColumnItem>, update, ((update as HierarchyUpdate).hierarchyGroupID == 0 ? this.parentSearchType : this.childSearchType));
                    }

                    // From Search
                    if ((update as MultiColumnListUpdate).values) {
                        if ((x[this.otherProductArray] as Array<HierarchyItem>).length > 0) this.editOtherItem(x[this.otherProductArray] as Array<HierarchyItem>, update, ((update as MultiColumnListUpdate).values![1].name == this.parentSearchType ? 0 : 1));
                        if ((x[this.otherProductSearchArray] as Array<MultiColumnItem>).length > 0) this.editOtherItem(x[this.otherProductSearchArray] as Array<MultiColumnItem>, update, (update as MultiColumnListUpdate).values![1].name);
                    }
                }
                ...
                if (update.type == ListUpdateType.Delete) {
                    ...
                    // From Hierarchy
                    if (update.deletedItems![0].hierarchyGroupID != null) {
                        if ((x[this.otherProductArray] as Array<HierarchyItem>).length > 0) this.deleteOtherItem(x[this.otherProductArray] as Array<HierarchyItem>, update.deletedItems![0], update.deletedItems![0].hierarchyGroupID!);
                        if ((x[this.otherProductSearchArray] as Array<MultiColumnItem>).length > 0) this.deleteOtherItem(x[this.otherProductSearchArray] as Array<MultiColumnItem>, update.deletedItems![0] as MultiColumnItem, (update.deletedItems![0].hierarchyGroupID == 0 ? this.parentSearchType : this.childSearchType));
                    }

                    // From Search
                    if ((update.deletedItems![0] as MultiColumnListUpdate).values) {
                        if ((x[this.otherProductArray] as Array<HierarchyItem>).length > 0) this.deleteOtherItem(x[this.otherProductArray] as Array<HierarchyItem>, update.deletedItems![0] as MultiColumnItem, (update.deletedItems![0] as MultiColumnItem).values![1].name == this.parentSearchType ? 0 : 1);
                        if ((x[this.otherProductSearchArray] as Array<MultiColumnItem>).length > 0) this.deleteOtherItem(x[this.otherProductSearchArray] as Array<MultiColumnItem>, update.deletedItems![0] as MultiColumnItem, (update.deletedItems![0] as MultiColumnItem).values![1].name);
                    }
                }
            })
        }
